...
impl Sq {
    pub(crate) fn new(params: &io_uring_params, ring_fd: i32) -> io::Result<Sq> {
        ...
        let sqes_mmap_sz: usize = params.sq_entries as usize * size_of::<io_uring_sqe>();
    ...
    fn flush(&mut self) -> u32 {
        ...
        let mut ktail = unsafe { &*self.ktail }.load(Acquire);

        for _ in 0..to_submit {
            let index = ktail & mask;
            self.array[index as usize].store(self.sqe_head & mask, Release);
            ktail += 1;
            self.sqe_head += 1;
        }

        let swapped = unsafe { &*self.ktail }.swap(ktail, Release);
    ...
    pub(crate) fn submit_all(&mut self, ring_flags: u32, ring_fd: i32) -> u64 {
        let submitted = if ring_flags & IORING_SETUP_SQPOLL == 0 {
            ...
            flushed
        } else if unsafe { &*self.kflags }.load(Acquire) & IORING_SQ_NEED_WAKEUP != 0 {
        ...
        assert_eq!(unsafe { &*self.kdropped }.load(Relaxed), 0);
        u64::from(submitted)
    }
}
